...
if [[ -x .venv/bin/matrix-opencode ]]; then
    bot_command=(.venv/bin/matrix-opencode)
elif command -v matrix-opencode >/dev/null 2>&1; then
    bot_command=(matrix-opencode)
else
    echo "matrix-opencode is not installed. Run: pip install -e ." >&2
    exit 1
fi
...
echo "OpenCode is ready; starting the Matrix bot. Press Ctrl-C to stop both."
"${bot_command[@]}" &
bot_pid=$!
